...
- [x] Rett relevante lint-funn og dokumenter endeleg vurderte unntak. Køyrd 11. september 2026: `lintDebug` gav **0 feil og 58 åtvaringar**; etter retting står **46 åtvaringar og framleis 0 feil**. Fjerna ni ubrukte strengressursar i begge språk, og retta eit brytpunkt som las `Configuration.screenHeightDp` i staden for vindauget — det gav feil Heim-oppsett i delt skjerm. To unntak er dokumenterte i koden med grunngjeving: `VectorRaster` på TV-banneret, som må vere 320 × 180 dp, og `PluralsCandidate` på «%d prosent», som ikkje bøyer seg i noko av språka. Dei attståande 46 er `InlinedApi` (13), `GradleDependency` og `NewerVersionAvailable` (16 avhengnadsoppdateringar), `UseKtx` (6) og `ModifierParameter` (6) — ingen av dei er åtferdsfeil. Dette er ikkje ein ferdig tryggleiks- eller kompatibilitetskontroll.
...
- [ ] **Detaljar:** Fast opningsgeometri, reserverte bilet- og tekstområde, roleg skeletonlasting og ein alltid tilgjengeleg lukkeknapp. Sein metadata skal ikkje flytte heile popupen. **12. september:** ein serie opnar tittelsida si i staden for eit rutenett av sesongmapper, og sida viser sesongar og episodar med stillbilete, lengd og framdrift. Plassen under mediebiletet ber no fakta, sjangrar og tagline; biletet er 340 dp og får eit uskarpt bakteppe av seg sjølv når forma ikkje passar ramma. [Omfang](docs/TV_REDESIGN_2026-09-12.md). **11. september:** sida viser lydspor, undertekstar og versjon før avspeling, og valet følgjer med inn i spelaren som ekstra på `JellyfinPlayerActivity`. Ho er samstundes stramma inn frå tretten stabla blokker til seks: framdrifta ligg inni «Hald fram»-knappen, sett og favoritt er ikon utan tekst, sju metadataplater er blitt éi dempa linje, og biletet har fått plassen som stod tom. [Grunngjeving per val](docs/LIBRARY_SHELVES_2026-09-11.md).
...
- [ ] **Bibliotek:** Filter og vising høyrer på skjermen, ikkje i ein popup med vala langs sida. Éi rad over rutenettet med det ein faktisk endrar ofte — sortering, sett/usett, sjanger — og ein eigen visingskontroll for **vising** (rutenett eller liste), **kortstorleik** og **bilettype** (plakat, thumb, banner, logo), slik Jellyfin har det. Vala blir lagra per bibliotek, så eit filmbibliotek kan ha plakatar og eit opptaksbibliotek thumbs. **Bygd 11. september 2026.** Både filter og vising ligg no på sida som kompakte chip-grupper, ikkje i ein popup. Filterdialogen leidde med eit søkefelt, og ein dialog gir fokus til første fokuserbare barn — så TV-tastaturet dekte halve dialogen i det du opna han. Inline stel ingenting fokus. Kvart val slår inn med ein gong; kladd med Bruk/Avbryt gav meining bak ein modal, ikkje med rutenettet rett bak. Søket skriv lokalt og sender ved Enter eller fokustap, ikkje per teikn. Vising, kortstorleik, bilettype og titlar av/på blir lagra per bibliotek-id (`LibraryDisplay`, seks einingstestar), og rutenettet måler celler etter `LibraryArtType.ratio`. `AUTO` følgjer medietypen og er standard. **11. september, andre bolk:** kvar bibliotekside hentar sine eigne «Hald fram å sjå» og «Neste episode» med `MediaServerClient.libraryShelves` — same førespurnaden som feeden, med eitt `view` i staden for alle. Hyllene ligg over rutenettet, berre på rota av eit bibliotek og berre utan aktive filter; ei filtrert side er eit søkeresultat. Hald inne eit kort for å fjerne det frå resume, setje favoritt eller markere sett; alle tre skriv til tenaren. `clearResume` prøver `UserItems/{id}/UserData`, så den brukarskopa stien, så ei stopp-melding på posisjon null, og går berre vidare ved 400/404/405. Snarvegane i menyen kan flyttast opp og ned, og rekkjefølgja blir lagra. [Omfang og grunngjeving](docs/LIBRARY_SHELVES_2026-09-11.md). **Att:** rulleposisjon ved skifte av bilettype er ikkje verifisert, og lista (`LibraryView.LIST`) er ikkje sett på eining.
...
- [x] Automatiske kontrollar av manglande omsetjingar, parameter og fleirtalsressursar er på plass (`TranslationParityTest`, 11. september 2026). Fire kontrollar les ressurs-XML-en direkte: streng finst i begge språk, ingen foreldrelause nynorsk-strengar, same formatparameter i begge (`%1$s` mot `%s` er eit krasj, ikkje ein skrivefeil), og same mengdeformer i fleirtalsressursar. Verifisert ved å leggje inn ein streng i berre eitt språk og sjå testen feile.

...
class LibrarySelectionPreferencesTest {
    @Test fun selectionsPersistPerServerAndProfileIncludingEmptyAndChangeCacheIdentity() {
        ...
        try {
            ...
            repo.setLibraryIcons(connection, mapOf("kids" to app.reelstack.data.model.LibraryIcon.KIDS))
            ...
            assertEquals(app.reelstack.data.model.LibraryIcon.KIDS, reopened.libraryIcons(connection)["kids"])
            assertTrue(reopened.libraryIcons(connection.copy(userId = "other")).isEmpty())
            assertTrue(reopened.libraryIcons(connection.copy(baseUrl = "https://other.example")).isEmpty())

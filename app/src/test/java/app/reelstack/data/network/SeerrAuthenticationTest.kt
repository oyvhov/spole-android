...
class SeerrAuthenticationTest {
    @Test fun jellyfinAccountUsesSeerrSessionAndCsrfWithoutForwardingJellyfinToken() {
        ...
        val connection = ServiceConnection(ServiceKind.SEERR, "Seerr", "https://seerr.example.com/base", login.accessToken, sessionCookie = true)
        SeerrServiceClient(transport).request(connection, "movie", 42)
        assertEquals(login.accessToken, transport.calls[2].headers["Cookie"])
    ...
    @Test fun sessionProbeChecksOwnAccountAndApiKeyStillWorks() {
        ...
        SeerrServiceClient(transport).request(account.copy(token = "admin-key", sessionCookie = false), "movie", 7)

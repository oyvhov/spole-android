...
class SeriesSeasonFlowTest {
    ...
    @Test fun futureSeasonShowsPremiereInsteadOfAnOverdueLibraryGap() {
        val season = RequestSeason(3, LocalizedText.raw("Sesong 3"), 8, 1, today.plusDays(10))
        assertTrue(season.canRequest)
        assertEquals(app.reelstack.R.string.season_airs_on, season.description(today).resId)
    }

    @Test fun undatedSeasonDoesNotPretendToHaveBeenReleased() {
        assertEquals(app.reelstack.R.string.season_premiere_unknown,
            RequestSeason(2, LocalizedText.raw(""), 0, 1).description(today).resId)
    }

    @Test fun airedSeasonCanBeMissingAndMayStillBeRequested() {
        assertEquals(app.reelstack.R.string.season_missing, RequestSeason(2, LocalizedText.raw(""), 8, 7, today).description(today).resId)
    }

    @Test fun availabilityAndRequestStatusTakePrecedenceOverPremiereMetadata() {
        for (status in 2..6) {
            val season = RequestSeason(2, LocalizedText.raw(""), 8, status, today.plusYears(1))
            assertEquals(season.label, season.description(today).resId)
    ...
    @Test fun nextEpisodeNeedsDateAndNumberAndNeverShowsOldNextDate() {
        ...
        assertEquals("S02 E03", next.description(today)!!.args[0])
    ...
    @Test fun onlyPartialAndAlreadyRequestedSeasonsOfferAvailabilityAlerts() {
        assertEquals(listOf(2, 3, 4), (0..8).filter { RequestSeason(1, LocalizedText.raw(""), 8, it).canWatch })
    }

    @Test fun localFollowIsNotReportedAsARequestOrDownload() {
        val partial = listOf(RequestSeason(1, LocalizedText.raw(""), 8, 4))
    ...
    @Test fun missingOrBlockedWatchStatusStaysUnknown() {
        ...
        assertEquals(RequestStage.UNKNOWN, availabilityWatchProgress(listOf(RequestSeason(1, LocalizedText.raw(""), 8, 4)), setOf(1), emptyList(), 6).stage)
    }

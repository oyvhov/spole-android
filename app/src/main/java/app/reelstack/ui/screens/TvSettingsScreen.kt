            ...
            SettingsCategory.entries.forEach { item ->
                WideDestination(stringResource(item.title), item.icon, category == item,
                    onClick = {
                        ...
                        category = item; enterPanel = television
                    },
                    modifier = Modifier.focusRequester(categoryFocus.getValue(item))
                        .onFocusChanged { if(television && it.isFocused) category = item }
                        .onPreviewKeyEvent {
        ...
        Column(Modifier.weight(1f).fillMaxHeight()) {
            ...
            key(category) { panes.SaveableStateProvider(category.name) {
                Column(Modifier.weight(1f).fillMaxWidth().focusRequester(panelFocus)
                    ...
                    SharedSettingsContent(category, state, onConnectionClick, onNotificationsChange, onWifiOnlyChange,
